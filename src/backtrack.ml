...
let get_new_item cur_c c item_idx items acc table =
  let cur_val = Point.get_point2 item_idx cur_c c table in
  let prev2_val = Point.get_point2 (item_idx - 1) cur_c c table in
...
let backtrack capacity items = function
  | None ->
    None
  | Some table ->
    ...
    let used_items = backtrack_aux y capacity x items acc table in
    Some used_items

let print = function
  | None ->
    Printf.eprintf "backtracking not possible: no data available\n";
    Printf.printf "\n"
  | Some items0 ->
